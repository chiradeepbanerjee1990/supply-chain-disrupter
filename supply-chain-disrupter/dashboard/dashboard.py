...
def show_scenario_analyzer() -> None:
    ...
    if result.weather_factors:
        with st.expander("🌡️ Weather Factor Details"):
            ...
            with col2:
                st.metric(
                    "Rain Score",
                    result.weather_factors.get("precipitation_score", 0)
                )
            with col3:
                st.metric(
                    "Weather Code Score",
                    result.weather_factors.get("weather_code_score", 0)
                )
            st.write(
                "Max Wind Speed:",
    ...
    if result.news_signals:
        with st.expander("📰 News Risk Signals Details"):
            st.write(f"Total signals found: {len(result.news_signals)}")

            avg_severity = sum(
                s.severity for s in result.news_signals
            ) / len(result.news_signals)
            st.metric("Average News Severity", round(avg_severity, 3))
                ...
                with st.expander(
                    ...
                    if signal.source_file:
                        ...
                        if signal.page_number:
                            source_text += f" (Page {signal.page_number})"
                        st.caption(source_text)

                    col1, col2, col3 = st.columns(3)
                    with col1:
                        st.write("🏢 Company:", signal.company or "Not detected")
                    with col2:
                        st.write("📍 Location:", signal.location or "Not detected")
                    with col3:
                        st.write("📅 Date:", signal.event_date or "Not detected")

                    st.caption(
                        f"Signal type: {signal.signal_type or 'unknown'} | "
                        f"Source type: {signal.source_type or 'unknown'} | "
                        f"Distance: {signal.retrieval_distance or 'N/A'}"
                    )
                    st.write("🏷️ Tags:", ", ".join(signal.signal_tags))
                    st.write("📝 Summary:")
