...
def weather_risk_monitoring_agent(state: GlobalState) -> Dict[str, Any]:
    ...
    from src.utils.api_clients import (
        compute_weather_factors,
        adjust_weather_for_disruption
    )

    # Get detailed weather factors
    weather_factors = compute_weather_factors(payload)

    # Adjust severity based on disruption type
    adjusted_severity = adjust_weather_for_disruption(
        weather_factors["severity"],
    ...
    log_message = (
        ...
        f"Adjusted severity: {adjusted_severity}, "
        f"Summary: {weather_factors['weather_summary']}"
    )
    ...
    return {
        ...
        "weather_factors": {
            ...
            "precipitation_score": weather_factors["precipitation_score"],
            "weather_code_score": weather_factors["weather_code_score"],
            "max_wind_speed": weather_factors["max_wind_speed"],
            "max_precipitation": weather_factors["max_precipitation"],
            "signal_type": "live_event",
        },
        "agent_logs": state.agent_logs + [log_message],
    }
...
def risk_classifier_agent(state: GlobalState) -> Dict[str, Any]:
    ...
    if news_signals:
        avg_news_severity = sum(
            s["severity"] for s in news_signals
        ) / len(news_signals)
    else:
    ...
    composite = min(1.0,
        ...
        (risk_inputs["weather_severity"] * 0.20) +
        (risk_inputs["chip_risk"] * 0.15) +
        (risk_inputs["supplier_risk"] * 0.15) +
    ...
    return {
        ...
        "agent_logs": state.agent_logs + [
            f"L4: Risk classification completed. "
            f"News avg severity: {round(avg_news_severity, 3)}, "
            f"Composite: {round(composite, 3)}, "
            f"Label: {label}"
        ],
    }
